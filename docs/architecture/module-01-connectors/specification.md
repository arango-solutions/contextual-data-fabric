...
- **FR-4 (P2 — pulled forward, due 2026-07-24):** Snowflake connector — metadata + pushdown query. ~~Gated on the free-tier check~~ **Gate resolved (2026-07-21):** a Snowflake account was available (a trial then; a paid, key-pair-authenticated account since); see PRD §7.7 and the P1 close-out plan Sprint 2. Pushdown rides the second Ontop endpoint over `snowflake-jdbc`; metadata via r2g's Phase-6 connector (RSA fallback).
